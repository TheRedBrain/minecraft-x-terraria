...
public class MxtPlayerInventory implements Inventory, Nameable, RecipeInputProvider {
...
//            this.main, this.coins, this.ammunition, this.trash, this.void_vault);
    ...
    public MxtPlayerInventory(PlayerEntity player) {
        this.player = player;
        // TODO config
        this.xGearInventoryOffset = 240;
        this.yGearInventoryOffset = 30;
        this.xEquipmentInventoryOffset = this.xGearInventoryOffset + 20;
    ...
    public int getEmptySlot(ItemStack stack) {
        ...
        for (int i = 40; i < this.main.size(); ++i) {
...
//        }
    ...
    public void updateItems() {
        ...
        if (((DuckPlayerEntityMixin)this.player).getActiveLoadout() == 0) {
...
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        ...
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.armor_dye_2.size(); ++i) {
            if (this.armor_dye_2.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.armor_dye_2.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.accessories_dye_2.size(); ++i) {
            if (this.accessories_dye_2.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.accessories_dye_2.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.armor_dye_3.size(); ++i) {
            if (this.armor_dye_3.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.armor_dye_3.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.accessories_dye_3.size(); ++i) {
            if (this.accessories_dye_3.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.accessories_dye_3.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        ...
        j = j + i;
        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.vanity_armor_2.size(); ++i) {
            if (this.vanity_armor_2.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.vanity_armor_2.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.vanity_accessories_2.size(); ++i) {
            if (this.vanity_accessories_2.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.vanity_accessories_2.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.vanity_armor_3.size(); ++i) {
            if (this.vanity_armor_3.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.vanity_armor_3.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.vanity_accessories_3.size(); ++i) {
            if (this.vanity_accessories_3.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.vanity_accessories_3.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
...
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        ...
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.armor_2.size(); ++i) {
            if (this.armor_2.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.armor_2.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.accessories_2.size(); ++i) {
            if (this.accessories_2.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.accessories_2.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.armor_3.size(); ++i) {
            if (this.armor_3.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.armor_3.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        j = j + i;
//        MinecraftXTerraria.LOGGER.info("j: " + j);
        for (i = 0; i < this.accessories_3.size(); ++i) {
            if (this.accessories_3.get(i).isEmpty()) continue;
            nbtCompound = new NbtCompound();
            nbtCompound.putByte("Slot", (byte)(i + j));
            this.accessories_3.get(i).writeNbt(nbtCompound);
            nbtList.add(nbtCompound);
        }
        return nbtList;
    }
...
//                continue;
//            }
////            if (j >= 59 && j < this.equipment_dye.size() + 59) {
////                this.equipment_dye.set(j - 59, itemStack);
////                continue;
////            }
////            if (j >= 64 && j < this.equipment.size() + 64) {
////                this.equipment.set(j - 64, itemStack);
////                continue;
////            }
////            if (j >= 69 && j < this.armor_dye.size() + 69) {
////                this.armor_dye.set(j - 69, itemStack);
////                continue;
////            }
////            if (j >= 72 && j < this.accessories_dye.size() + 72) {
////                this.accessories_dye.set(j - 72, itemStack);
////                continue;
////            }
////            if (j >= 79 && j < this.vanity_armor.size() + 79) {
////                this.vanity_armor.set(j - 79, itemStack);
////                continue;
////            }
////            if (j >= 82 && j < this.vanity_accessories.size() + 82) {
////                this.vanity_accessories.set(j - 82, itemStack);
////                continue;
////            }
////            if (j >= 89 && j < this.armor.size() + 89) {
////                this.armor.set(j - 89, itemStack);
////                continue;
////            }
////            if (j >= 92 && j < this.accessories.size() + 92) {
////                this.accessories.set(j - 92, itemStack);
////                continue;
////            }
...
//    }
    ...
    public void readNbt(NbtList nbtList) { // TODO nbt
        ...
        for (int i = 0; i < nbtList.size(); ++i) {
            ...
            if (j >= 192 && j < this.accessories_3.size() + 192) {
                this.accessories_3.set(j - 192, itemStack);
            }
        }
    }
