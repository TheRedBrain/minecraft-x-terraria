...
public abstract class ClientPlayNetworkHandlerMixin implements MxtClientPlayPacketListener {
...
//    }
    ...
    public void onScreenHandlerSlotUpdate(ScreenHandlerSlotUpdateS2CPacket packet) {
        ...
        if (packet.getSyncId() == ScreenHandlerSlotUpdateS2CPacket.UPDATE_CURSOR_SYNC_ID) {
            ...
            ((DuckPlayerEntityMixin)playerEntity).getMxtPlayerInventory().setStack(i, itemStack);
        } else {
            boolean bl = false;
            ...
            if (packet.getSyncId() == 0 && MxtPlayerInventoryScreenHandler.isInHotbar(i)) {
                ItemStack itemStack2;
                if (!itemStack.isEmpty() && ((itemStack2 = ((DuckPlayerEntityMixin)playerEntity).getMxtPlayerInventoryScreenHandler().getSlot(i).getStack()).isEmpty() || itemStack2.getCount() < itemStack.getCount())) {
                    itemStack.setBobbingAnimationTime(5);
                }
                ((DuckPlayerEntityMixin)playerEntity).getMxtPlayerInventoryScreenHandler().setStackInSlot(i, packet.getRevision(), itemStack);
    ...
    /**
     * @author TheRedBrain
     * @reason use custom playerScreenHandler
     */
    @Overwrite
    public void onInventory(InventoryS2CPacket packet) {
        ...
        if (packet.getSyncId() == 0) {
            ((DuckPlayerEntityMixin)playerEntity).getMxtPlayerInventoryScreenHandler().updateSlotStacks(packet.getRevision(), packet.getContents(), packet.getCursorStack());
